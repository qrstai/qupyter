...
class AsyncCallLimiter:
    def __init__(self):
        self.semaphore = asyncio.Semaphore(_MAX_CALLS)
        self.windows = {_SCOPE: []}

    async def wait_limit_rate(self, max_calls=_MAX_CALLS, period=_PERIOD, scope=_SCOPE):
        if scope not in self.windows:
            self.windows[scope] = []

        async with self.semaphore:
            now = time.time()
            window = [t for t in self.windows[scope] if now - t < period]
            window_size = len(window)
...
def limit_async_calls(max_calls=_MAX_CALLS, period=_PERIOD, scope=_SCOPE):
    """
    여러 함수에 대한 호출을 제한하는 데코레이터입니다.
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            await AsyncCallLimiter().wait_limit_rate(max_calls=max_calls, period=period, scope=scope)
